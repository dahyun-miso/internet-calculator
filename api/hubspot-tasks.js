...
import { getOwnerIdByEmail, searchTodayTasksByOwner } from '../lib/hubspot.js';
...
export default async function handler(request) {
  ...
  if (!email) {
    return new Response(JSON.stringify({ tasks: [], mapped: false }), { headers: jsonHeaders });
  }

  try {
    const ownerId = await getOwnerIdByEmail(email);
    if (!ownerId) {
      return new Response(JSON.stringify({ tasks: [], mapped: true, ownerFound: false }), { headers: jsonHeaders });
    }
    const tasks = await searchTodayTasksByOwner(ownerId);
    return new Response(JSON.stringify({ tasks }), { headers: jsonHeaders });
  } catch (e) {
    console.error('hubspot-tasks error', e);
