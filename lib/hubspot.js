...
// "INT 최근(2일) 상담" 뷰 재현: [INT] 재제안 대상 목록에 속하면서, 다음 활동 날짜가 오늘이거나 아예 없는 전체 거래 (소유자 무관)
export async function searchIntRecentDeals() {
  const { startUtc, endUtc } = kstTodayRangeMs();
    ...
    const res = await fetch(`${HUBSPOT_BASE}/crm/v3/objects/deals/search`, {
      ...
      body: JSON.stringify({
        filterGroups: [
          {
            filters: [
              { propertyName: 'notes_next_activity_date', operator: 'BETWEEN', value: String(startUtc), highValue: String(endUtc) },
            ],
          },
          {
            filters: [
        ...
        properties: ['dealname', 'dealstage', 'notes_next_activity_date', NO_ANSWER_COUNT_PROPERTY],
        sorts: [{ propertyName: 'notes_next_activity_date', direction: 'ASCENDING' }],
        limit: 100,
        ...(after ? { after } : {}),
      }),
    });
    if (!res.ok) throw new Error(`int-recent deals search failed: ${res.status}`);
    const data = await res.json();
    deals.push(...(data.results || [])
      .filter((d) => memberIds.has(String(d.id)))
      .map((d) => ({
        id: d.id,
        dealname: d.properties?.dealname || '',
        dealstage: d.properties?.dealstage || '',
  ...
  const stageMap = await getDealStageMap();
  deals.forEach((d) => { d.dealstageLabel = stageMap[d.dealstage]?.label || d.dealstage || '(단계 없음)'; });
  return deals;
}
